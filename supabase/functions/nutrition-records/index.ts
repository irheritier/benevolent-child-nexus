...
serve(async (req) => {
  ...
  try {
    ...
    // Le dernier élément devrait être l'ID de l'enfant
    ...
    const nutritionStatus = searchParams.get('nutrition_status');
    ...
    if (nutritionStatus) {
      query = query.eq('nutrition_status', nutritionStatus);
    }

    // Exécuter la query
    const { data: nutritionRecords, error: recordsError, count } = await query;

    if (recordsError) {
      console.error('Erreur récupération records nutritionnels:', recordsError)
      return new Response(
        JSON.stringify({ 
          error: 'Erreur lors de la récupération des records nutritionnels',
    ...
    const metrics = {
      ...
      has_next_page: (from + limit) < (count || 0),
